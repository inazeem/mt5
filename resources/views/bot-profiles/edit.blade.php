<x-app-layout>
    <x-page-header title="Edit Bot Profile" :subtitle="'Profile: ' . ($profile['name'] ?? 'Bot')" />
            ...
            <x-card>
                <form method="POST" action="{{ route('bot-profiles.update', $profile['key']) }}" class="space-y-6">
                    ...
                    <div class="rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-3">
                        <p class="text-sm font-semibold text-indigo-900">Edit profile behavior safely</p>
                        <p class="text-xs text-indigo-700 mt-1">Only non-empty fields override global defaults. Keep fields blank to inherit shared bot settings.</p>
                    </div>

                    <div class="p-3 bg-gray-50 rounded text-xs text-gray-600 font-mono border border-gray-200">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                        ...
                        <div>
                            ...
                            <input type="number" name="adx_min_floor" step="0.1" min="1" max="100"
                                   value="{{ old('adx_min_floor', isset($profile['adx_min_floor']) ? $profile['adx_min_floor'] : '') }}" placeholder="e.g. 22 (forex default)"
                                   class="mt-1 block w-full rounded border-gray-300" />
                    ...
                    <div class="space-y-2">
                        ...
                        <label class="flex items-center gap-2">
                            <input type="checkbox" id="reverse_strategy" name="reverse_strategy" value="1" {{ old('reverse_strategy', (bool) ($profile['reverse_strategy'] ?? false)) ? 'checked' : '' }} class="rounded border-gray-300" />
                            <span class="text-sm text-gray-700">Reverse strategy side (buy/sell inversion)</span>
                        </label>
                        <label class="flex items-center gap-2">
                            <input type="checkbox" id="use_adx_score" name="use_adx_score" value="1" {{ old('use_adx_score', array_key_exists('use_adx_score', $profile) ? (bool) $profile['use_adx_score'] : true) ? 'checked' : '' }} class="rounded border-gray-300" />
                    ...
                    @php
                        $selectedTickerCategories = old('ticker_categories');
                        if (!is_array($selectedTickerCategories)) {
                            $selectedTickerCategories = isset($profile['ticker_categories']) && is_array($profile['ticker_categories'])
                                ? $profile['ticker_categories']
                                : [];
                        }
                    @endphp
                                    ...
                                    @foreach (['Forex', 'Stock', 'Commodity', 'Index', 'Crypto', 'Other'] as $tickerCategory)
                                        <label class="inline-flex items-center gap-2 rounded border border-indigo-100 bg-white px-3 py-2">
                                            <input type="checkbox" name="ticker_categories[]" value="{{ $tickerCategory }}"
                                                   {{ in_array($tickerCategory, $selectedTickerCategories, true) ? 'checked' : '' }}
